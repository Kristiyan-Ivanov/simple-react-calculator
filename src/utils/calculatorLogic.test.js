...
describe('replaceOperator', () => {
  test.each(['+', '-', '*', '/'])(
    "replaces %s operator with '-'",
    (operator) => {
      const overrideOperator = '-';
      expect(
        logic.replaceOperator(shortDisplayValue + operator, overrideOperator),
      ).toBe(shortDisplayValue + overrideOperator);
    },
  );

  test('does not replace operator if last character is not an operator', () => {
    const operator = '+';
    expect(logic.replaceOperator(shortDisplayValue, operator)).toBe(
      shortDisplayValue + operator,
    );
  });
});
...
describe('getLastNumber', () => {
  test.each([
    ...
    'returns the last number in the display value',
...
describe('numberCount', () => {
  test.each([
    ...
    'returns the correct number of numbers in the display value',
    ({ displayValue, expected }) => {
